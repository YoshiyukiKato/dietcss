...
function validateRules(rules, doms, whitelist){
  ...
  removeUnusedKeyframes(rules, keyframes);
}
...
function removeUnusedKeyframes(rules, keyframesMayUnused){
  rules.forEach((rule) => {
    if(rule.type === "rule"){
      const animationDeclaration = rule.declarations.find((declaration) => {
        return declaration.property.match(/(-.+-)animation-name$/) || declaration.property.match(/(-.+-)animation$/);
      });
      if(animationDeclaration){
        keyframesMayUnused = keyframesMayUnused.filter((keyframes) => {
          return !animationDeclaration.value.match(keyframes.name)
        });
      };
    }else if(rule.type === "media"){
      removeUnusedKeyframes(rule.rules, keyframesMayUnused);
    }else{
      //nothing todo
    }
  });

  keyframesMayUnused.forEach(keyframeUnused => {
    const index = rules.indexOf(keyframeUnused);
    rules.splice(index, 1);
  })
}
